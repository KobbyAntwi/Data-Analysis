...
app.layout = html.Div([
    html.H1('Groups360 Security Dashboard', style={'textAlign': 'center'}),
    dcc.Upload(
        id='upload-data',
        children=html.Div([
            'Drag and Drop or ',
...
def create_graphs(filtered_data, selected_col, all_data):
    if filtered_data is not None:
        ...
        if list(dff.columns)[0] == 'Year': #change year
            print(dff.head())
        ...
        elif list(dff.columns)[0] == 'SEASON':
            dff = dff.groupby(['year'])[['']]
            fig_bots =px.pie(dff,names='countriesAndTerritories', values=piedropval, hole=.3,labels={'countriesAndTerritories':'Countries'})
    else:
        return {}
